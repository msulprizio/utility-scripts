...
!
      ...
      CHARACTER(LEN=78), PARAMETER :: BLANKV=' '
!
...
!   Check whether a comment line and if so copy to buffer.
      ...
      USER .OR. LINE == ' ' .OR. LINE(:1) == 'c'                       &
         ...
         IF (KNTCOM == KKLIM) THEN
            WRITE (NOUT , '(A78)') (CBUF(78*L5-71:78*L5) , L5 = 1 ,    &
            KNTCOM) , LINE
            KNTCOM = 0
         ...
         ELSE
            KNTCOM = KNTCOM+1
            CBUF(78*KNTCOM-71:78*KNTCOM) = LINE
         END IF
         GO TO 2
      END IF
!
!   Some form of embedded comment?
      ...
      DO L22 = 2, 78
         ...
         IF (.NOT. INTFL) THEN
            IF (KNTCOM  <  KKLIM) THEN
               KNTCOM = KNTCOM +1
               CBUF(78*KNTCOM-71:78*KNTCOM) =                          &
                                BLANKV(:L22-1)//LINE(L22:78)
            ELSE
               WRITE (NOUT, '(A)') BLANKV(:L22-1)//LINE(L22:78)
            END IF
         END IF
         LINE(L22:78) = ' '
         IF (LINE  ==  ' ') GO TO 2
...
!   Set counters for new program unit
               ...
               KNTCON = 0
               LENST = 0
               IF (KNTCOM /= 0) WRITE (NOUT , '(A78)') (CBUF(78*L5-71: &
               78*L5) , L5 = 1 , KNTCOM)
               KNTCOM = 0
               NAME = ' '
               NOARG = 0
...
!   Write any comments following statement
   11 IF (KNTCOM /= 0) THEN
         WRITE (NOUT ,'(A78)') (CBUF(78*L5-71:78*L5) , L5 = 1 , KNTCOM)
         KNTCOM = 0
      END IF
!
...
!*****************************************************************************80
!
!! CONVERT is the main program for converting F77 to F90.
!
